...
2. **Excess replica shedding** (`scheduleExcessReplicaShedding`): If `len(region.Peers) > maxPeerCount`, removes the peer on the most-loaded store (by region count via `meta.GetRegionCountPerStore()`). Returns a `ChangePeer{RemoveNode}` command.
...
flowchart TD
    ...
    ES -- Yes --> REM[Return ChangePeer RemoveNode]
...
- `StartMove(move PendingMove)` -- records a new pending move for the given region.
...
- `MockClient` -- full in-memory implementation with 14 test cases. Includes `GetAllStores`.
