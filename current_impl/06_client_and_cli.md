...
5. **`commitPrimary(ctx)`** — sends `KvCommit` RPC for the primary key synchronously via `SendToRegion`. Skips if 1PC already committed.
6. On commit failure: checks if primary was actually committed via **`isPrimaryCommitted(ctx)`** — calls `KvCheckTxnStatus` to verify. If primary IS committed (Raft committed but response was lost due to timeout/leader change), proceeds with `commitSecondaries` and returns success. Otherwise calls `rollback()` and returns the error.
7. **`commitSecondaries(ctx)`** — commits each secondary key individually via `SendToRegion` (one RPC per key, not batched by region). This ensures correct routing after splits. If `TxnLockNotFound` is returned, retries up to 5 times (to handle Raft replication delay); after 5 retries, accepts as genuinely resolved. Errors are logged via `slog.Warn` but do not fail the transaction (primary is already committed).
